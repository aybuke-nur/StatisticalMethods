...
geometric_pmf <- function(k, p) {
  ...
  # in slides k is the number of tries (1, 2, ...).
  # R's dgeom function uses the failure number(x = k-1)
  # Formula: p * (1-p)^(k-1)
...
geometric_cdf <- function(q, p) {
  ...
  # CDF: P(X <= q)
  # if q < 1 then the prob. is 0
  # R's pgeom function uses number of failures so we give (q-1)
...
geometric_rnd <- function(n, p) {
  ...
  # we add +1 to the result to find the number of success
